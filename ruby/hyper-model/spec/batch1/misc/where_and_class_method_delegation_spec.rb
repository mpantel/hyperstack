...
RSpec::Steps.steps 'the where method and class delegation', js: true do
  ...
  it "and will update the collection on the client " do
    # The previous step proves the collection LOADED, not that the pusher
    # handshake finished. Broadcasting while the client is still pending queues
    # the message against a connection that expires in 10s, and it is then
    # deleted unread -- which is what made this spec fail on most CI runs. (#70)
